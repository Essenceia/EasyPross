...
/**
 * \class ProbeModel_Abstract ProbeModel_Abstract.java
 * "Model/AbstractClasses/ProbeModel_Abstract.java" \brief Direct child of
 * ObjectModel_Absract and asbtract class from which prob classes inherite.
 * <p>
 * \details Probes are the the inputs and outputs at the expremities of the
 * graph, they are the points where we give our initial data and get the data
 * from at the end of the simulation.
 * <p>
 * \note Addes the necessary attributes for a probe sutch as the width of the
 * connected wire and a buffer for the data on that wire.
 */
public abstract class ProbeModel_Abstract extends ObjectModel_Abstract {
    ...
    /**
     * * < Cache for the data the probe contains, will not change unless signal
 ...
 /*public Vector<WireModel> getWire() {
     ...
     * \fn setter wire_size
     *
     * \note if setData has not been overwritten you are not supposed to be
     * using it, for instance a end probe is not suposed to be setting the data
     * on it's line, is is supposed to be a read only.
     *
     * @param data
     */
