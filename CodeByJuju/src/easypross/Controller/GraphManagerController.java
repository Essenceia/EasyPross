...
import easypross.Model.Logic.Gate.ANDModel;
import easypross.Model.Logic.Gate.ORModel;
import easypross.Model.Probe.*;
...
public class GraphManagerController implements easypross.Interface.GraphManagerInterface{
    ...
    private void create_all_wires(){
        ...
        for(Map.Entry<Integer,WireModel> mapentry : GArrettes.entrySet()){
   ...
   private void read_line(String toread) {
    ...
    private void createLGate(String[] line,Integer type){
        ...
        switch (type){
            case 2:GNoeuds.put(id,new ANDModel(id,in_wire,out_wire));
            break;
            case 3:GNoeuds.put(id,new ORModel(id,in_wire,out_wire));
            break;
        }
    }
    private void createProb(String[] line,Integer type){
        ...
        switch (type){
            case 4:GDebut.put(id,new ProbeStartModel(id,wire));
                break;
            case 5:GFin.put(id,new ProbeEndModel(id,wire));
                break;
        }
    }

    private void readGNoeuds(){
        for(Map.Entry<Integer,NodeModel_Abstract> mapentry : GNoeuds.entrySet()){
            System.out.println(mapentry.getValue().getDescription());
        }
    }
    private void readProbs(){
        System.out.println("Prob Start :");
        for(Map.Entry<Integer,ProbeStartModel> mapentry : GDebut.entrySet()){
            System.out.println(mapentry.getValue().getDescription());
        }
        System.out.println("Prob End :");
        for(Map.Entry<Integer,ProbeEndModel> mapentry : GFin.entrySet()){
            System.out.println(mapentry.getValue().getDescription());
        }
    }
